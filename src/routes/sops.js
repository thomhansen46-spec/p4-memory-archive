...
const DB = path.join(__dirname, '../data/sops.json');
const load = () => fs.existsSync(DB) ? JSON.parse(fs.readFileSync(DB)) : [];
const save = (data) => fs.writeFileSync(DB, JSON.stringify(data, null, 2));

module.exports = (app) => {
  app.get('/sops', (req, res) => res.json(load()));

  app.get('/sops/:id', (req, res) => {
    const record = load().find(r => r.id === req.params.id);
    record ? res.json(record) : res.status(404).json({ error: 'Not found' });
  });

  app.post('/sops', (req, res) => {
    const record = { id: uuidv4(), record_type: 'sop_record', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), ...req.body };
    const data = load();
    data.push(record);
    save(data);
